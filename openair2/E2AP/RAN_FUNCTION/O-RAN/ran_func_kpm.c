...
{
  if (b_sd == NULL) {
    /* SST-only condition: match any UE with this SST, regardless of SD */
...
static arr_ue_id_t filter_ues_by_s_nssai_in_du_or_monolithic(const test_info_lst_t test_info)
{
  ...
  UE_iterator(RC.nrmac[0]->UE_info.connected_ue_list, ue) {
    ...
    // [perf] printf("[KPM-DU-DBG] UE rnti=%04x lc_count=%zu\n", ue->rnti, seq_arr_size(&sched_ctrl->lc_config));
    ...
    for (size_t l = 0; l < seq_arr_size(&sched_ctrl->lc_config); ++l) {
      ...
      // [perf] printf("[KPM-DU-DBG]   lcid=%u nssai={sst=%u sd=0x%x}\n", c->lcid, c->nssai.sst, c->nssai.sd);
...
/* Collect all connected UEs for the current node type, without any filtering.
 * Used by the FORMAT_1_ACTION_DEFINITION path. */
static arr_ue_id_t collect_all_ues(void)
{
  arr_ue_id_t arr = {0};
  arr.ue_id = calloc(MAX_MOBILES_PER_GNB, sizeof(ue_id_e2sm_t));
  assert(arr.ue_id != NULL);

  const ngran_node_t node_type = get_e2_node_type();
  printf("[E2 AGENT][E2SM-KPM] collect_all_ues: node_type=%d RC.nrmac=%p\n",
         node_type, (void*)RC.nrmac);

  switch (node_type) {
    case ngran_gNB: {
      if (RC.nrmac == NULL || RC.nrmac[0] == NULL) break;
      arr.ue_info_list = calloc(MAX_MOBILES_PER_GNB, sizeof(*arr.ue_info_list));
      assert(arr.ue_info_list != NULL);
      UE_iterator(RC.nrmac[0]->UE_info.connected_ue_list, ue) {
        rrc_gNB_ue_context_t *rrc_ue_ctx = rrc_gNB_get_ue_context_by_rnti(RC.nrrrc[0], -1, ue->rnti);
        if (!rrc_ue_ctx || rrc_ue_ctx->ue_context.amf_ue_ngap_id >= (1LL << 40))
          continue;
        arr.ue_id[arr.sz] = fill_ue_id_data[ngran_gNB](&rrc_ue_ctx->ue_context, 0, 0);
        arr.ue_info_list[arr.sz] = ue;
        arr.sz++;
        AssertFatal(arr.sz < MAX_MOBILES_PER_GNB, "too many UEs\n");
      }
      break;
    }
    case ngran_gNB_CU: {
      struct rrc_gNB_ue_context_s *rrc_ue_ctx = NULL;
      RB_FOREACH(rrc_ue_ctx, rrc_nr_ue_tree_s, &RC.nrrrc[0]->rrc_ue_head) {
        gNB_RRC_UE_t *ue = &rrc_ue_ctx->ue_context;
        if (ue->amf_ue_ngap_id >= (1LL << 40))
          continue;
        arr.ue_id[arr.sz] = fill_ue_id_data[ngran_gNB_CU](ue, 0, 0);
        arr.sz++;
        AssertFatal(arr.sz < MAX_MOBILES_PER_GNB, "too many UEs\n");
      }
      break;
    ...
    case ngran_gNB_DU: {
      if (RC.nrmac == NULL || RC.nrmac[0] == NULL) {
        printf("[E2 AGENT][E2SM-KPM] DU: RC.nrmac not ready\n");
        break;
      }
      arr.ue_info_list = calloc(MAX_MOBILES_PER_GNB, sizeof(*arr.ue_info_list));
      assert(arr.ue_info_list != NULL);
      size_t mac_ue_count = 0;
      UE_iterator(RC.nrmac[0]->UE_info.connected_ue_list, ue) {
        mac_ue_count++;
        f1_ue_data_t rrc_ue_id = du_get_f1_ue_data(ue->rnti);
        arr.ue_id[arr.sz] = fill_ue_id_data[ngran_gNB_DU](NULL, rrc_ue_id.secondary_ue, 0);
        arr.ue_info_list[arr.sz] = ue;
        arr.sz++;
        AssertFatal(arr.sz < MAX_MOBILES_PER_GNB, "too many UEs\n");
      }
      printf("[E2 AGENT][E2SM-KPM] DU: MAC UEs iterated=%zu collected=%zu\n", mac_ue_count, arr.sz);
      break;
    }
    case ngran_gNB_CUUP: {
      ue_id_t *pdcp_ue_id_list = calloc(MAX_MOBILES_PER_GNB, sizeof(ue_id_t));
      assert(pdcp_ue_id_list != NULL);
      arr.sz = nr_pdcp_get_num_ues(pdcp_ue_id_list, MAX_MOBILES_PER_GNB);
      for (size_t i = 0; i < arr.sz; i++)
        arr.ue_id[i] = fill_ue_id_data[ngran_gNB_CUUP](NULL, 0, pdcp_ue_id_list[i]);
      free(pdcp_ue_id_list);
      break;
    }
    default:
      printf("[E2 AGENT][E2SM-KPM] Format 1 AD: node type %d not supported for UE collection\n", node_type);
      break;
  }
...
bool read_kpm_sm(void* data)
{
  ...
  switch (kpm->act_def->type) {
    case FORMAT_1_ACTION_DEFINITION: {
      // Action Definition Format 1 → report all connected UEs with Format 3 Indication
      kpm->ind.hdr = kpm_ind_hdr();
      kpm->ind.msg.type = FORMAT_3_INDICATION_MESSAGE;

      arr_ue_id_t arr_ue_id = collect_all_ues();
      if (arr_ue_id.sz == 0) {
        printf("[E2 AGENT][E2SM-KPM] Format 1 AD: no UEs connected.\n");
        free(arr_ue_id.ue_id);
        free(arr_ue_id.ue_info_list);
        return false;
      }
      kpm->ind.msg.frm_3 = fill_kpm_ind_msg_frm_3(&arr_ue_id, &kpm->act_def->frm_1);
      free(arr_ue_id.ue_id);
      free(arr_ue_id.ue_info_list);
      break;
    }
