...
static void fill_rc_control(ran_func_def_ctrl_t* ctrl)
{
  ...
  /* ---- Style 1: Radio Bearer Control (Action 2: QoS flow mapping) ---- */
  ...
  /* ---- Style 3: Connected Mode Mobility (Action 1: HO trigger) ---- */
  seq_ctrl_style_t* ctrl_style3 = &ctrl->seq_ctrl_style[1];

  ctrl_style3->style_type = 3;

  const char control_name3[] = "Connected Mode Mobility";
  ctrl_style3->name = cp_str_to_ba(control_name3);

  ctrl_style3->hdr = FORMAT_1_E2SM_RC_CTRL_HDR;
  ctrl_style3->msg = FORMAT_1_E2SM_RC_CTRL_MSG;
  ctrl_style3->call_proc_id_type = NULL;
  ctrl_style3->out_frmt = FORMAT_1_E2SM_RC_CTRL_OUT;

  ctrl_style3->sz_seq_ctrl_act = 1;
  ctrl_style3->seq_ctrl_act = calloc(ctrl_style3->sz_seq_ctrl_act, sizeof(seq_ctrl_act_2_t));
  assert(ctrl_style3->seq_ctrl_act != NULL && "Memory exhausted");

  seq_ctrl_act_2_t* ctrl_act3 = &ctrl_style3->seq_ctrl_act[0];
  ctrl_act3->id = 1;

  const char control_act_name3[] = "Handover Control";
  ctrl_act3->name = cp_str_to_ba(control_act_name3);

  ctrl_act3->sz_seq_assoc_ran_param = 1;
  ctrl_act3->assoc_ran_param = calloc(ctrl_act3->sz_seq_assoc_ran_param, sizeof(seq_ran_param_3_t));
  assert(ctrl_act3->assoc_ran_param != NULL && "Memory exhausted");

  ctrl_act3->assoc_ran_param[0].id = 1;
  const char ran_param_rnti[] = "RNTI";
  ctrl_act3->assoc_ran_param[0].name = cp_str_to_ba(ran_param_rnti);
  ctrl_act3->assoc_ran_param[0].def = NULL;

  ctrl_style3->sz_ran_param_ctrl_out = 0;
  ctrl_style3->ran_param_ctrl_out = NULL;
}
...
//  assert(data->type == RAN_CONTROL_CTRL_V1_03 );
  ...
  const uint32_t style  = ctrl->hdr.frmt_1.ric_style_type;
  const uint16_t act_id = ctrl->hdr.frmt_1.ctrl_act_id;

#if defined(NGRAN_GNB_CUCP)
  if (style == 3 && act_id == 1) {
    /* CHO xApp: trigger intra-CU F1 handover.
     * RAN Parameter 1 = RNTI (integer, key-flag false). */
    const seq_ran_param_t* ran_param = ctrl->msg.frmt_1.ran_param;
    assert(ctrl->msg.frmt_1.sz_ran_param >= 1 && "Style 3 Act 1 requires at least 1 RAN parameter (RNTI)");
    assert(ran_param[0].ran_param_id == 1 && "Style 3 Act 1: RAN Parameter 1 must be RNTI");
    int64_t rnti_val = 0;
    if (ran_param[0].ran_param_val.type == ELEMENT_KEY_FLAG_TRUE_RAN_PARAMETER_VAL_TYPE)
      rnti_val = ran_param[0].ran_param_val.flag_true->int_ran;
    else if (ran_param[0].ran_param_val.type == ELEMENT_KEY_FLAG_FALSE_RAN_PARAMETER_VAL_TYPE)
      rnti_val = ran_param[0].ran_param_val.flag_false->int_ran;
    else
      AssertFatal(false, "[E2 RC] Style 3 Act 1: unexpected RAN param value type %d\n", ran_param[0].ran_param_val.type);
    uint16_t rnti = (uint16_t)rnti_val;
    printf("[E2 RC] Style 3 Act 1: CHO HO trigger for RNTI 0x%04x\n", rnti);
    bool ok = nr_rrc_e2_trigger_ho(rnti);
    if (!ok)
      LOG_W(NR_RRC, "[E2 RC] HO trigger rejected (UE busy or not found)\n");

  } else
#endif
  if (act_id == 2) {
    /* Style 1 Act 2: QoS flow mapping configuration */
    ...
    printf("[E2 RC] Unsupported control: style=%u act_id=%u\n", style, act_id);
    assert(0 != 0 && "Unsupported RC control style/action");
  }
