                                             ...
                                             const NR_ReconfigurationWithSync_t *reconfWithSync)
{
  ...
  LOG_A(NR_MAC,
        "UE reconfigurationWithSync: target CRNTI=0x%04x target PCI=%ld\n",
        mac->crnti,
        reconfWithSync->spCellConfigCommon && reconfWithSync->spCellConfigCommon->physCellId
            ? *reconfWithSync->spCellConfigCommon->physCellId
            : -1);
  ...
  if (reconfWithSync->rach_ConfigDedicated) {
    ...
    LOG_A(NR_MAC,
          "UE reconfigurationWithSync: dedicated RACH present (%s)\n",
          ra->rach_ConfigDedicated && ra->rach_ConfigDedicated->cfra ? "CFRA" : "no CFRA");
  } else {
    LOG_W(NR_MAC, "UE reconfigurationWithSync: no dedicated RACH, target RA will fall back to CBRA with C-RNTI CE\n");
  }
