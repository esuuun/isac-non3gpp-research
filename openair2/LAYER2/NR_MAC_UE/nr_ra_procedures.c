...
static void configure_ra_preamble(NR_UE_MAC_INST_t *mac)
{
  ...
  if (ra->ra_type == RA_4_STEP) {
    ...
    if (ra->pdcch_order.active && ra->pdcch_order.preamble_index != 0xb000000) {
      ...
      if (ssb < 0 && ssb_list->ssb_ResourceList.list.count > 0) {
        NR_CFRA_SSB_Resource_t *res = ssb_list->ssb_ResourceList.list.array[0];
        ssb = res->ssb;
        ra->ra_PreambleIndex = res->ra_PreambleIndex;
        ra->ro_mask_index = ssb_list->ra_ssb_OccasionMaskIndex;
        LOG_W(MAC,
              "CFRA SSB mismatch: mib_ssb=%d not in dedicated list, fallback ssb=%d preamble=%ld\n",
              mac->mib_ssb,
              ssb,
              ra->ra_PreambleIndex);
      }
    ...
    if (ra->cfra) {
      ...
      if (ssb < 0 && cfra->resourcesTwoStep_r16.ssb_ResourceList.list.count > 0) {
        NR_CFRA_SSB_Resource_t *res = cfra->resourcesTwoStep_r16.ssb_ResourceList.list.array[0];
        ssb = res->ssb;
        ra->ra_PreambleIndex = res->ra_PreambleIndex;
        ra->ro_mask_index = cfra->resourcesTwoStep_r16.ra_ssb_OccasionMaskIndex;
        LOG_W(MAC,
              "2-step CFRA SSB mismatch: mib_ssb=%d not in dedicated list, fallback ssb=%d preamble=%ld\n",
              mac->mib_ssb,
              ssb,
              ra->ra_PreambleIndex);
      }
