...
void nr_mac_prepare_ra_ue(gNB_MAC_INST *nrmac, NR_UE_info_t *UE)
{
  ...
  NR_COMMON_channels_t *cc = &nrmac->common_channels[0];
  ra->preambles.num_preambles = 0;
  for (int i = 0; i < cc->num_active_ssb; i++) {
    for (int j = 0; j < cfra->resources.choice.ssb->ssb_ResourceList.list.count; j++) {
      NR_CFRA_SSB_Resource_t *ssb_res = cfra->resources.choice.ssb->ssb_ResourceList.list.array[j];
      if (cc->ssb_index[i] == ssb_res->ssb) {
        // one dedicated preamble for each beam
        DevAssert(ra->preambles.num_preambles < MAX_NUM_NR_PRACH_PREAMBLES);
        ra->preambles.preamble_list[ra->preambles.num_preambles++] = ssb_res->ra_PreambleIndex;
        LOG_I(NR_MAC,
              "F1HO CFRA: UE RNTI %04x SSB %d dedicated preamble %ld\n",
              UE->rnti,
              cc->ssb_index[i],
              ssb_res->ra_PreambleIndex);
        break;
      }
    }
  }
  LOG_I(NR_MAC,
        "Added new %s process for UE RNTI %04x with initial CellGroup (%d dedicated preambles)\n",
        ra->cfra ? "CFRA" : "CBRA",
        UE->rnti,
        ra->preambles.num_preambles);
}
