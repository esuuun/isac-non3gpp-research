  ...
  UE_iterator(nr_mac->UE_info.access_ue_list, UE) {
    ...
    for (int i = 0; i < ra->preambles.num_preambles; ++i) {
      if (ra->preambles.preamble_list[i] == preamble_index) {
        LOG_I(NR_MAC,
              "Matched preconfigured RA context: preamble %u -> UE RNTI 0x%04x (%s)\n",
              preamble_index,
              UE->rnti,
              ra->cfra ? "CFRA" : "CBRA");
        return UE;
                         ...
                         uint32_t preamble_power)
{
  ...
  if (!UE) {
    ...
    UE_iterator(nr_mac->UE_info.access_ue_list, access_UE) {
      NR_RA_t *access_ra = access_UE->ra;
      if (!access_ra || access_ra->preambles.num_preambles == 0)
        continue;
      LOG_W(NR_MAC,
            "RA preamble %u did not match preconfigured UE RNTI 0x%04x (%s, %d preambles)\n",
            preamble_index,
            access_UE->rnti,
            access_ra->cfra ? "CFRA" : "CBRA",
            access_ra->preambles.num_preambles);
      for (int i = 0; i < access_ra->preambles.num_preambles; i++)
        LOG_W(NR_MAC, "  preconfigured preamble[%d]=%u\n", i, access_ra->preambles.preamble_list[i]);
    }
    rnti_t rnti;
  ...
  LOG_D(NR_MAC, "%d.%d UE RA-RNTI %04x TC-RNTI %04x: initiating RA procedure\n", frame, slot, ra->RA_rnti, UE->rnti);
                             ...
                             nfapi_nr_tx_data_request_t *TX_req)
{
  ...
  LOG_D(NR_MAC, "%d.%d Send RAR to RA-RNTI %04x\n", frameP, slotP, ra->RA_rnti);
...
void nr_release_ra_UE(gNB_MAC_INST *mac, rnti_t rnti)
{
  ...
  if (UE) {
    if (UE->CellGroup && UE->ra && UE->ra->cfra && UE->ra->ra_state != nrRA_gNB_IDLE && du_exists_f1_ue_data(rnti)) {
      NR_RA_t *ra = UE->ra;
      LOG_W(NR_MAC,
            "Preserve F1HO target CFRA context for UE RNTI 0x%04x after RA failure; waiting for next target RA attempt\n",
            rnti);
      ra->ra_state = nrRA_gNB_IDLE;
      ra->RA_rnti = 0;
      ra->Msg3_tda_id = 0;
      ra->contention_resolution_timer = 0;
      ra->preamble_frame = 0;
      ra->preamble_slot = 0;
      ra->preamble_index = 0;
      ra->timing_offset = 0;
      NR_SCHED_LOCK(&UE_info->mutex);
      bool readded = add_UE_to_list(NR_NB_RA_PROC_MAX, UE_info->access_ue_list, UE);
      NR_SCHED_UNLOCK(&UE_info->mutex);
      if (readded)
        return;
      LOG_E(NR_MAC, "Could not preserve F1HO target CFRA context for UE RNTI 0x%04x; deleting it\n", rnti);
    }
