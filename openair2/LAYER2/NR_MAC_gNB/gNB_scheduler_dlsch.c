...
#ifdef E2_AGENT
#include "../../E2AP/RAN_FUNCTION/CUSTOMIZED/nr_slice_sched.h"
#endif
/*MAC*/
...
bwp_info_t get_pdsch_bwp_start_size(gNB_MAC_INST *mac, NR_UE_info_t *UE)
{
  bwp_info_t bwp_info;
  if (!UE) {
    // SIB/BCH case: use coreset0 BWP if configured, otherwise initial DL BWP from SCC
    if (mac->cset0_bwp_size != 0) {
      ...
      uint32_t loc_bw = scc->downlinkConfigCommon->initialDownlinkBWP->genericParameters.locationAndBandwidth;
      bwp_info.bwpStart = NRRIV2PRBOFFSET(loc_bw, MAX_BWP_SIZE);
      bwp_info.bwpSize = NRRIV2BW(loc_bw, MAX_BWP_SIZE);
    }
    return bwp_info;
  }
  ...
  // contiguously allocated non-interleaved or interleaved virtual resource blocks within the active bandwidth part of size N_BWP PRBs
  ...
  if (dl_bwp->dci_format == NR_DL_DCI_FORMAT_1_0 && sched_ctrl->search_space->searchSpaceType
    ...
    if (mac->cset0_bwp_size > 0) {
      bwp_info.bwpSize = min(dl_bwp->BWPSize - bwp_info.bwpStart, mac->cset0_bwp_size);
    } else {
                                                     ...
                                                     int pdu_index)
{
  ...
  LOG_D(NR_MAC,
        "prepare_pdsch_pdu: rnti=0x%x pduIdx=%d BWP(%d+%d) SCS=%d rb(%d+%d) mcs=%d R=%d Qm=%d TBS=%d rv=%d "
        "nrOfLayers=%d dlDmrsSymbPos=0x%x dmrsType=%d numDmrsCdmGrpsNoData=%d tda(%d,%d)\n",
        rnti, pdu_index,
        pdsch_pdu->BWPStart, pdsch_pdu->BWPSize, pdsch_pdu->SubcarrierSpacing,
        pdsch_pdu->rbStart, pdsch_pdu->rbSize,
        pdsch_pdu->mcsIndex[0], pdsch_pdu->targetCodeRate[0], pdsch_pdu->qamModOrder[0],
        pdsch_pdu->TBSize[0], pdsch_pdu->rvIndex[0],
        pdsch_pdu->nrOfLayers, pdsch_pdu->dlDmrsSymbPos, pdsch_pdu->dmrsConfigType,
        pdsch_pdu->numDmrsCdmGrpsNoData,
        pdsch_pdu->StartSymbolIndex, pdsch_pdu->NrOfSymbols);
                  ...
                  int n_rb_sched[num_beams])
{
  ...
  while (iterator->UE != NULL) {
    ...
    bwp_info_t bwp_info = sched_pdsch->bwp_info;
    int rbStart = 0; // WRT BWP start
    int rbStop = bwp_info.bwpSize - 1;
    int bwp_start = bwp_info.bwpStart;
#ifdef E2_AGENT
    uint16_t slice_rb_limit = 0;
    /* Enforce STATIC slice RB range for this UE */
    {
      uint16_t s_low = 0, s_high = 0;
      if (nr_slice_get_rb_range(rnti, &s_low, &s_high)) {
        if ((int)s_low  > rbStart) rbStart = (int)s_low;
        if ((int)s_high < rbStop)  rbStop  = (int)s_high;
        if (rbStop >= rbStart)
          slice_rb_limit = (uint16_t)(rbStop - rbStart + 1);
      }
    }
#endif
    ...
    int eff_min_rbSize = min_rbSize;
#ifdef E2_AGENT
    /* Enforce NVS capacity / MINMAX slice: cap max_rbSize to the slice's PRB budget */
    {
      uint16_t slice_max = nr_slice_get_max_rbs(rnti, (uint16_t)bwp_info.bwpSize);
      if (slice_max > 0) {
        slice_rb_limit = slice_max;
        if (max_rbSize > slice_max)
          max_rbSize = slice_max;
      }
      /* MINMAX floor: when this UE is scheduled, give it at least min PRBs. */
      uint16_t slice_min = nr_slice_get_min_rbs(rnti, (uint16_t)bwp_info.bwpSize);
      if (slice_min > eff_min_rbSize)
        eff_min_rbSize = slice_min;
    }
#endif
...
#ifdef E2_AGENT
    /* max_rbSize >= min_rbSize here; bound the slice floor to available RBs */
    if (eff_min_rbSize > max_rbSize)
      eff_min_rbSize = max_rbSize;
#endif
...
#ifdef E2_AGENT
    if (slice_rb_limit > 0)
      nr_slice_note_sched_activation(rnti, frame, slot, sched_pdsch->rbStart, sched_pdsch->rbSize, slice_rb_limit);
#endif
