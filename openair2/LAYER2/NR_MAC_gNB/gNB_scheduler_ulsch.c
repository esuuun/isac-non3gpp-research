...
static void handle_msg3_failed_rx(gNB_MAC_INST *mac, NR_RA_t *ra, rnti_t rnti, int harq_round_max)
{
  ...
  if (ra->cfra) {
    LOG_D(NR_MAC, "UE %04x RA failed at state %s (NSA msg3 reception failed)\n", rnti, nrra_text[ra->ra_state]);
    nr_release_ra_UE(mac, rnti);
    return;
  }

  if (ra->msg3_round >= harq_round_max - 1) {
    LOG_D(NR_MAC, "UE %04x RA failed at state %s (Reached msg3 max harq rounds)\n", rnti, nrra_text[ra->ra_state]);
    nr_release_ra_UE(mac, rnti);
    return;
  }
                         ...
                         const uint16_t rssi)
{
  ...
  if (ra->cfra) {
    ...
    if (crnti != 0) { // 3GPP TS 38.321 Table 7.1-1: RNTI values, RNTI 0x0000: N/A
      ...
      bool old_UE_in_ra = false;
      if (!old_UE) {
        /* During F1 handover, the target DU creates the UE context from
         * F1AP UE Context Setup and keeps it in access_ue_list until target
         * RA completes.  The UE identifies that context with the C-RNTI CE
         * in Msg3, so look in the RA/access list before dropping Msg3. */
        old_UE = find_ra_UE(&mac->UE_info, crnti);
        old_UE_in_ra = old_UE != NULL;
      }
      if (!old_UE) {
        ...
        if (du_exists_f1_ue_data(crnti))
          LOG_W(NR_MAC, "  F1 mapping for C-RNTI 0x%04x still exists, but MAC UE context is not in access/connected list\n", crnti);
        UE_iterator(mac->UE_info.access_ue_list, access_UE)
          LOG_W(NR_MAC, "  access_ue_list has RNTI 0x%04x\n", access_UE->rnti);
        UE_iterator(mac->UE_info.connected_ue_list, connected_UE)
          LOG_W(NR_MAC, "  connected_ue_list has RNTI 0x%04x\n", connected_UE->rnti);
        nr_release_ra_UE(mac, rnti);
        return;
      }
      if (old_UE_in_ra)
        LOG_I(NR_MAC, "Resolved C-RNTI 0x%04x to target RA UE context\n", crnti);
      ...
      if (old_UE_in_ra) {
        LOG_I(NR_MAC,
              "F1HO target RA completed with C-RNTI 0x%04x; skip duplicate DU-triggered RRC Reconfiguration\n",
              crnti);
      } else if (!old_UE->reconfigSpCellConfig) {
      ...
      nr_release_ra_UE(mac, rnti);
      if (old_UE_in_ra && !transition_ra_connected_nr_ue(mac, old_UE)) {
        LOG_E(NR_MAC, "cannot transition target RA UE %04x to connected: list is full\n", old_UE->rnti);
        delete_nr_ue_data(old_UE, NULL, &mac->UE_info.uid_allocator);
        return;
      }
