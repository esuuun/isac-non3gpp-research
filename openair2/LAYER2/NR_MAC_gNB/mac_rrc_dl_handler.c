                                        ...
                                        const nr_rlc_configuration_t *rlc_config)
{
  ...
  *resp_drbs = calloc(drbs_len, sizeof(**resp_drbs));
  ...
  for (int i = 0; i < drbs_len; i++) {
    ...
    LOG_D(NR_MAC, "[MAC-DRB] UE rnti=%04x DRB id=%d lcid=%d nssai={sst=%u sd=0x%x}\n",
           UE->rnti, drb->id, c.lcid, c.nssai.sst, c.nssai.sd);
    int prio = 100;
...
static NR_UE_info_t *create_new_UE(gNB_MAC_INST *mac, uint32_t cu_id, const NR_CG_ConfigInfo_t *cgci)
{
  ...
  LOG_A(NR_MAC,
        "F1 UE Context Setup: created target UE context rnti=0x%04x uid=%d pci=%ld cu_ue_id=%u\n",
        UE->rnti,
        UE->uid,
        scc && scc->physCellId ? *scc->physCellId : -1,
        cu_id);
...
void dl_rrc_message_transfer(const f1ap_dl_rrc_message_t *dl_rrc)
{
  ...
  if (UE == NULL) {
    /* CPE_UE check: the UE context was injected by the CU without going
     * through RACH, so the DU MAC may not yet have a scheduler entry.
     * If the RNTI is in the CPE_UE mapping table, treat it as valid and
     * let the RLC layer queue the PDU; the UE entry will be created when
     * the CPE_UE's Contention-Free RACH completes. */
    cpe_ue_table_t *ctbl = cpe_ue_table_get();
    bool is_cpe_ue_rnti = false;
    if (ctbl) {
      cpe_ue_lock(ctbl);
      is_cpe_ue_rnti = (cpe_ue_find_by_rnti_locked(ctbl, dl_rrc->gNB_DU_ue_id) != NULL);
      cpe_ue_unlock(ctbl);
    }
    if (is_cpe_ue_rnti) {
      LOG_I(NR_MAC,
            "[CPE_UE] DL RRC for injected RNTI %04x — forwarding directly to RLC\n",
            dl_rrc->gNB_DU_ue_id);
      pthread_mutex_unlock(&mac->sched_lock);
      /* Skip MAC scheduler check; deliver directly to RLC/SRB */
      ...
      nr_rlc_srb_recv_sdu(dl_rrc->gNB_DU_ue_id, dl_rrc->srb_id,
                          dl_rrc->rrc_container, dl_rrc->rrc_container_length);
      return;
    }
