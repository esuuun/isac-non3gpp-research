...
#include "nr_rlc/nr_rlc_configuration.h"
...
void *nrmac_stats_thread(void *arg) {
  ...
  while (oai_exit == 0) {
    ...
    p += snprintf(p, end - p, "\n");
    p += print_meas_log(&gNB->gNB_scheduler, "DL & UL scheduling timing", NULL, NULL, p, end - p);
                      ...
                      const nr_rlc_configuration_t *default_rlc_config)
{
  ...
  if (RC.nb_nr_macrlc_inst > 0) {
    ...
    int rlc_init_success = nr_rlc_module_init(mode);
    AssertFatal(rlc_init_success == 0, "Could not initialize NR RLC layer\n");

    // These should be out of here later
    if (get_softmodem_params()->usim_test == 0 ) nr_pdcp_layer_init();

    if(IS_SOFTMODEM_NOS1 && get_softmodem_params()->phy_test) {
      // get default noS1 configuration (EPS bearer ID=10, DRB ID=1, LCID=4, no ciphering)
      const nr_rlc_configuration_t *rlc_conf = &RC.nrmac[0]->rlc_config;
      const nr_pdcp_configuration_t default_pdcp_config = {.drb = {.sn_size = 12, .t_reordering = 0, .discard_timer = -1}};
      NR_RadioBearerConfig_t *rbconfig =
          get_default_rbconfig(10, 1, NR_CipheringAlgorithm_nea0, NR_SecurityConfig__keyToUse_master,
                               &default_pdcp_config);
      AssertFatal(rbconfig != NULL, "get_default_rbconfig() failed\n");
      NR_RLC_BearerConfig_t *rlc_rbconfig = get_DRB_RLC_BearerConfig(4, 1, NR_RLC_Config_PR_um_Bi_Directional, 1, rlc_conf);
...
void mac_top_destroy_gNB(gNB_MAC_INST *mac)
{
  ...
  pthread_mutex_destroy(&mac->UE_info.mutex);
  free(mac->tag);
  free(mac);
}
