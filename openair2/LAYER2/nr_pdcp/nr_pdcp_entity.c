                                    ...
                                    char *_buffer, int size)
{
  ...
  if (rcvd_count < entity->rx_deliv
    ...
    static uint64_t old_count_discards = 0;
    static uint64_t duplicate_discards = 0;
    const bool old_count = rcvd_count < entity->rx_deliv;
    const bool duplicate = nr_pdcp_sdu_in_list(entity->rx_list, rcvd_count);
    uint64_t *counter = old_count ? &old_count_discards : &duplicate_discards;
    (*counter)++;
    if (*counter <= 10 || *counter == 100 || *counter == 1000 || *counter % 10000 == 0) {
      LOG_W(PDCP,
            "discard NR PDU reason=%s rcvd_count=%u rx_deliv=%u rx_next=%u gap=%d duplicate=%d total=%lu\n",
            old_count ? "old_count" : "duplicate",
            rcvd_count,
            entity->rx_deliv,
            entity->rx_next,
            old_count ? (int)(entity->rx_deliv - rcvd_count) : 0,
            duplicate,
            *counter);
    }
    entity->stats.rxpdu_dd_pkts++;
...
{
  /* transmitting entity: reset COUNT so UE and gNB stay in sync when the
   * UE PDCP has restarted (e.g. softmodem restart or RLF recovery). */
  entity->tx_next = 0;

  /* receiving entity: flush reorder buffer and reset all state variables */
  if (entity->t_reordering_start != 0)
    deliver_all_sdus(entity);
  entity->t_reordering_start = 0;
  entity->rx_next  = 0;
