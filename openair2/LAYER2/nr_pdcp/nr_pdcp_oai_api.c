             ...
             const nr_pdcp_entity_security_keys_and_algos_t *security_parameters)
{
  ...
  nr_pdcp_manager_lock(nr_pdcp_ue_manager);
  ue = nr_pdcp_manager_get_ue(nr_pdcp_ue_manager, UEid);
  nr_pdcp_entity_t *existing_rb = nr_pdcp_get_rb(ue, drb_id, false);
  if (existing_rb != NULL) {
    /* DRB already exists — UE reconnected without proper context release.
     * Reset PDCP COUNT so the stale rx_deliv does not discard the UE's
     * new UL packets (which start from COUNT=0 after the UE restarted). */
    LOG_W(PDCP, "DRB %d already exists for UE %ld, resetting PDCP state (rx_deliv was %u)\n",
          drb_id, UEid, existing_rb->rx_deliv);
    existing_rb->reestablish_entity(existing_rb, &actual_security_parameters);
    existing_rb->tx_next          = 0;
    existing_rb->rx_next          = 0;
    existing_rb->rx_deliv         = 0;
    existing_rb->rx_reord         = 0;
    existing_rb->t_reordering_start = 0;
  } else {
