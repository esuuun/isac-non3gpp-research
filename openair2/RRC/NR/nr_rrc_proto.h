...
/*
 * Walk the RRC UE tree looking for a UE whose IMSI matches.
 * Returns the C-RNTI of the CPE device, or 0 if not found.
 * Note: IMSI must have been stored in gNB_RRC_UE_t.imsi (added in nr_rrc_defs.h).
 */
uint16_t rrc_gNB_find_rnti_by_imsi(uint64_t imsi);

/*
 * Inject a new RRC UE context for CPE_UE without a full RACH.
 * Allocates a C-RNTI, creates a minimal UE context on the DU
 * that the CPE_UE will use for Contention-Free RACH.
 * Sets ctx->rnti on success.
