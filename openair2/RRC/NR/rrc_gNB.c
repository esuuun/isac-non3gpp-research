...
#include "common/utils/http_server/cpe_ho_api.h"
#include "common/utils/http_server/cpe_rnti_ws.h"
...
#include "alg/find.h"
#include "openair2/RRC/NR/rrc_gNB_cpe_nas.h"
...
static int64_t f1ho_now_us(void)
{
  struct timespec ts;
...
static void f1ho_post_latency_to_xapp(const char *json)
{
  const char *xapp_ip = getenv("F1HO_XAPP_IP");
  if (!xapp_ip || !xapp_ip[0])
    return;

  int port = 8082;
  const char *port_s = getenv("F1HO_XAPP_PORT");
  if (port_s && port_s[0])
    port = atoi(port_s);
  ...
  if (inet_pton(AF_INET, xapp_ip, &addr.sin_addr) != 1) {
    close(fd);
    return;
  }
...
static void handle_rrcReconfigurationComplete(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, const NR_RRCReconfigurationComplete_t *reconfig_complete)
{
  ...
  switch (UE->xids[xid]) {
    ...
    case RRC_PDUSESSION_ESTABLISH:
      if (UE->n_initial_pdu > 0) {
         ...
         * Depth B: CPE_UE has no AMF context — skip NGAP response. */
        if (!UE->is_cpe_ue) {
          rrc_gNB_send_NGAP_INITIAL_CONTEXT_SETUP_RESP(rrc, UE);
        } else {
          for (int _i = 0; _i < UE->nb_of_pdusessions; _i++)
            if (UE->pduSession[_i].status == PDU_SESSION_STATUS_DONE)
              UE->pduSession[_i].status = PDU_SESSION_STATUS_ESTABLISHED;
        }
        UE->n_initial_pdu = 0;
  ...
  if (UE->ho_context != NULL) {
    ...
    UE->ho_context->rrc_reconf_complete_us = f1ho_now_us();

    LOG_A(NR_RRC,
          "[F1HO_LAT] phase=rrc_reconf_complete ue=%u rnti=0x%04x old_rnti=0x%04x target_rnti=0x%04x "
          "trigger_to_complete_us=%ld reconf_to_complete_us=%ld f1_setup_req_to_resp_us=%ld "
          "trigger_to_f1_setup_resp_us=%ld t_us=%ld\n",
          UE->rrc_ue_id,
          UE->rnti,
          UE->ho_context->source ? UE->ho_context->source->old_rnti : 0,
          UE->ho_context->target->new_rnti,
          UE->ho_context->trigger_us ? UE->ho_context->rrc_reconf_complete_us - UE->ho_context->trigger_us : -1,
          UE->ho_context->rrc_reconf_send_us ? UE->ho_context->rrc_reconf_complete_us - UE->ho_context->rrc_reconf_send_us : -1,
          (UE->ho_context->f1_setup_req_us && UE->ho_context->f1_setup_resp_us)
              ? UE->ho_context->f1_setup_resp_us - UE->ho_context->f1_setup_req_us
              : -1,
          UE->ho_context->trigger_us && UE->ho_context->f1_setup_resp_us
              ? UE->ho_context->f1_setup_resp_us - UE->ho_context->trigger_us
              : -1,
          UE->ho_context->rrc_reconf_complete_us);

    char lat_json[1024];
    snprintf(lat_json,
             sizeof(lat_json),
             "{\"ue\":%u,\"rnti\":\"0x%04x\",\"old_rnti\":\"0x%04x\",\"target_rnti\":\"0x%04x\","
             "\"trigger_to_complete_us\":%ld,\"reconf_to_complete_us\":%ld,"
             "\"f1_setup_req_to_resp_us\":%ld,\"trigger_to_f1_setup_resp_us\":%ld}",
             UE->rrc_ue_id,
             UE->rnti,
             UE->ho_context->source ? UE->ho_context->source->old_rnti : 0,
             UE->ho_context->target->new_rnti,
             ...
             UE->ho_context->trigger_us && UE->ho_context->f1_setup_resp_us
                 ? UE->ho_context->f1_setup_resp_us - UE->ho_context->trigger_us
                 : -1);
    f1ho_post_latency_to_xapp(lat_json);
  ...
  if (ul_dcch_msg->message.present == NR_UL_DCCH_MessageType_PR_c1) {
    ...
    switch (ul_dcch_msg->message.choice.c1->present) {
      ...
      case NR_UL_DCCH_MessageType__c1_PR_securityModeComplete:
        ...
        /* Depth B: CPE_UE uses local NAS proxy — send Registration Accept directly */
        if (UE->is_cpe_ue) {
          rrc_gNB_cpe_nas_security_complete(rrc, UE);
          break;
        }
      ...
      case NR_UL_DCCH_MessageType__c1_PR_securityModeFailure:
        ...
        if (UE->is_cpe_ue) {
          /* CPE_UE has no NAS-derived KgNB so it always rejects RRC SecurityModeCommand.
           * With NEA0/NIA0 (null security) we can skip AS security and proceed. */
          LOG_I(NR_RRC,
                "[CPE NAS] UE %d: SecurityModeFailure ignored (NEA0/NIA0) — proceeding to Registration Accept\n",
                UE->rrc_ue_id);
          UE->as_security_active = true;
          rrc_gNB_cpe_nas_security_complete(rrc, UE);
        } else {
...
static void rrc_CU_process_ue_context_setup_response(MessageDef *msg_p, instance_t instance)
{
  ...
  if (UE->ho_context == NULL) {
    ...
    UE->ho_context->f1_setup_resp_us = f1ho_now_us();
    UE->ho_context->target->du_ue_id = resp->gNB_DU_ue_id;
    UE->ho_context->target->new_rnti = *resp->crnti;
    LOG_A(NR_RRC,
          "[F1HO_LAT] phase=f1_setup_response ue=%u old_rnti=0x%04x target_rnti=0x%04x target_du_ue_id=%u "
          "f1_setup_req_to_resp_us=%ld trigger_to_f1_setup_resp_us=%ld t_us=%ld\n",
          UE->rrc_ue_id,
          UE->rnti,
          UE->ho_context->target->new_rnti,
          UE->ho_context->target->du_ue_id,
          UE->ho_context->f1_setup_req_us ? UE->ho_context->f1_setup_resp_us - UE->ho_context->f1_setup_req_us : -1,
          UE->ho_context->trigger_us ? UE->ho_context->f1_setup_resp_us - UE->ho_context->trigger_us : -1,
          UE->ho_context->f1_setup_resp_us);
...
/* =========================================================================
 * CPE UE Handover helpers
 * =========================================================================
 *
 * rrc_gNB_find_rnti_by_imsi()
 * ---------------------------
 * Walk the RRC UE context tree and return the C-RNTI of the UE whose
 * gNB_RRC_UE_t.imsi field matches the supplied value.
 *
 * NOTE: imsi is populated by rrc_gNB_process_NGAP_NAS_FIRST_REQ() (or
 * equivalent) when the AMF sends the IMSI in the Initial UE Message.
 * If the CPE has not yet attached, 0 is returned.
 * ========================================================================= */
uint16_t rrc_gNB_find_rnti_by_imsi(uint64_t imsi)
{
  if (imsi == 0) return 0;
  ...
  rrc_gNB_ue_context_t *ue_p = NULL;
  RB_FOREACH(ue_p, rrc_nr_ue_tree_s, &rrc->rrc_ue_head) {
    if (ue_p->ue_context.imsi == imsi)
      return (uint16_t)ue_p->ue_context.rnti;
  }
  return 0;
}
 ...
 * ------------------------------------
 * Inject a synthetic UE context for a CPE_UE without going through RACH.
 *
 * Strategy:
 *   1. Choose a target DU — use the DU that owns the CPE's own context
 *      (looked up by cpe_rnti), falling back to the first available DU.
 *   2. Pick a free RNTI in the reserved CPE_UE range [0xE000, 0xEFFF].
 *      (The DU must honour Contention-Free RACH preamble + this RNTI.)
 *   3. Create a minimal RRC UE context (rrc_gNB_create_ue_context).
 *   4. Mark the context as is_cpe_ue = 1.
 *   5. Activate SRB1 so the DU can handle the first RRC message.
 *
 * Returns the allocated RNTI, or 0 on failure.
 * ========================================================================= */

#define CPE_UE_RNTI_BASE   0xE000u
#define CPE_UE_RNTI_LIMIT  0xEFFFu

uint16_t rrc_gNB_allocate_rnti_for_cpe_ue(cpe_ue_entry_t *ctx)
{
  if (!ctx) return 0;

  gNB_RRC_INST *rrc = RC.nrrrc[0];
  if (!rrc) {
    LOG_E(NR_RRC, "[CPE HO] RC.nrrrc[0] not initialized\n");
    return 0;
  }

  /* Step 1: find the target DU.
   * Prefer the DU that already serves the CPE device (same assoc_id as
   * the existing CPE context), so the CPE_UE ends up on the same cell. */
  sctp_assoc_t target_assoc = 0;

  if (ctx->cpe_rnti != 0) {
    /* look up CPE's existing context to find its DU assoc_id */
    rrc_gNB_ue_context_t *cpe_ctx =
      rrc_gNB_get_ue_context_by_rnti_any_du(rrc, ctx->cpe_rnti);
    if (cpe_ctx) {
      f1_ue_data_t d = cu_get_f1_ue_data(cpe_ctx->ue_context.rrc_ue_id);
      target_assoc = d.du_assoc_id;
    }
  }

  /* Fall back to the first DU in the tree */
  if (target_assoc == 0) {
    nr_rrc_du_container_t *first_du = RB_MIN(rrc_du_tree, &rrc->dus);
    if (!first_du) {
      LOG_E(NR_RRC, "[CPE HO] no DU available for CPE_UE injection\n");
      return 0;
    }
    target_assoc = first_du->assoc_id;
  }

  /* Step 2: find a free RNTI in the CPE_UE reserved range */
  uint16_t new_rnti = 0;
  for (uint16_t r = CPE_UE_RNTI_BASE; r <= CPE_UE_RNTI_LIMIT; r++) {
    if (rrc_gNB_get_ue_context_by_rnti(rrc, target_assoc, r) == NULL) {
      new_rnti = r;
      break;
    }
  }
  if (new_rnti == 0) {
    LOG_E(NR_RRC, "[CPE HO] CPE_UE RNTI pool exhausted\n");
    return 0;
  }

  /* Step 3: create synthetic UE context.
   * Use imsi as the ue_identity so the tree key is unique.
   * du_ue_id is set equal to the RNTI (same convention as normal RACH). */
  rrc_gNB_ue_context_t *new_ctx =
    rrc_gNB_create_ue_context(target_assoc, new_rnti, rrc,
                               ctx->imsi, (uint32_t)new_rnti);
  if (!new_ctx) {
    LOG_E(NR_RRC, "[CPE HO] rrc_gNB_create_ue_context failed for CPE_UE\n");
    return 0;
  }

  /* Step 4: mark as CPE_UE */
  gNB_RRC_UE_t *UE = &new_ctx->ue_context;
  UE->is_cpe_ue = 1;
  UE->imsi      = ctx->imsi;

  /* Step 5: activate SRB1 so DU can forward the first RRC PDU */
  activate_srb(UE, 1);

  LOG_I(NR_RRC,
        "[CPE HO] injected CPE_UE context: RNTI=0x%04x rrc_ue_id=%u DU_assoc=%d imsi=%llu\n",
        new_rnti, UE->rrc_ue_id, target_assoc,
        (unsigned long long)ctx->imsi);
