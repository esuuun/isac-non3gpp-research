...
#ifdef E2_AGENT
static bool extract_imsi_from_nas_pdu(const uint8_t *buf, size_t len, char *imsi_out, size_t imsi_out_size);
static void write_imsi_to_db(uint64_t ue_id, const char *imsi);
#endif
...
void rrc_gNB_send_NGAP_NAS_FIRST_REQ(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, NR_RRCSetupComplete_IEs_t *rrcSetupComplete)
{
...
#ifdef E2_AGENT
  /* LBO: try to extract IMSI from SUCI in plain Registration Request */
  if (lbo_is_enabled()) {
    char imsi[20];
    if (extract_imsi_from_nas_pdu(rrcSetupComplete->dedicatedNAS_Message.buf,
                                   rrcSetupComplete->dedicatedNAS_Message.size,
                                   imsi, sizeof(imsi)))
      write_imsi_to_db(UE->rrc_ue_id, imsi);
    else
      LOG_D(NR_RRC, "[LBO] UE %u: no plain-SUCI IMSI in NAS PDU (GUTI re-reg or security protected)\n",
            UE->rrc_ue_id);
  }
#endif
  ...
  /* Depth B: if the IMSI in the Registration Request belongs to a CPE_UE,
   * intercept here and run the local NAS proxy instead of forwarding to AMF. */
  {
    ...
    if (extract_imsi_from_nas_pdu(rrcSetupComplete->dedicatedNAS_Message.buf,
      ...
      cpe_ue_table_t *cpe_tbl = cpe_ue_table_get();
      cpe_ue_lock(cpe_tbl);
      cpe_ue_entry_t *cpe_e = cpe_ue_find_by_imsi_locked(cpe_tbl, imsi);
      bool is_cpe = (cpe_e != NULL && cpe_e->valid);
      if (is_cpe)
        cpe_e->rnti = UE->rnti; /* store RNTI now so wait_rnti can find it */
      cpe_ue_unlock(cpe_tbl);

      if (is_cpe) {
        LOG_I(NR_RRC,
              "[CPE NAS] UE %u: IMSI %llu matched CPE_UE table — Depth B intercept\n",
              UE->rrc_ue_id, (unsigned long long)imsi);
        UE->imsi      = imsi;
        UE->is_cpe_ue = 1;
        /* Free the NAS PDU buffer allocated by create_byte_array() above */
        free(req->nas_pdu.buf);
        req->nas_pdu.buf = NULL;
        itti_free(ITTI_MSG_ORIGIN_ID(message_p), message_p);
        rrc_gNB_cpe_nas_init(rrc, UE);
        return;
      }
      LOG_I(NR_RRC,
            "[CPE NAS] UE %u: IMSI %llu not in CPE_UE table — forward to AMF\n",
            UE->rrc_ue_id, (unsigned long long)imsi);
    } else {
      LOG_I(NR_RRC,
            "[CPE NAS] UE %u: could not extract IMSI from Registration Request (NAS len=%zu) — forward to AMF\n",
            UE->rrc_ue_id, rrcSetupComplete->dedicatedNAS_Message.size);
    }
  }
...
void rrc_gNB_send_NGAP_UPLINK_NAS(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, const NR_UL_DCCH_Message_t *const ul_dcch_msg)
{
  ...
  /* Depth B: handle UL NAS locally for CPE_UE, skip NGAP forwarding. */
  if (UE->is_cpe_ue) {
    rrc_gNB_cpe_nas_handle_ul_nas(rrc, UE, nas->buf, nas->size);
    return;
  }
...
void rrc_gNB_send_NGAP_PDUSESSION_SETUP_RESP(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE, uint8_t xid)
{
  ...
  for (int pdusession = 0; pdusession < UE->nb_of_pdusessions; pdusession++) {
    ...
    if (session->status == PDU_SESSION_STATUS_DONE) {
      ...
      /* Register outer CPE UE RNTI + rrc_ue_id so CPE_UE DRB setup can find
       * it directly and CU-UP can fall back to the outer CPE's DRB for DL
       * while the CPE_UE's own DRB (gtp_ready) is not yet established. */
      if (!UE->is_cpe_ue)
        cpe_ue_table_register_outer_cpe(UE->rnti, UE->rrc_ue_id);
      char ip_str[INET_ADDRSTRLEN] = {0};
...
#ifdef E2_AGENT
/* Extract IMSI from a plain-text 5G NAS Registration Request (TS 24.501).
 * Only succeeds for SUCI with null protection scheme (unencrypted MSIN). */
static bool extract_imsi_from_nas_pdu(const uint8_t *buf, size_t len, char *imsi_out, size_t imsi_out_size)
{
  /* Plain Registration Request layout:
   * buf[0] = 0x7e  (5GMM EPD)
   * buf[1] = 0x00  (plain security header)
   * buf[2] = 0x41  (Registration Request)
   * buf[3] = ngKSI(bits7-4) | 5GS_reg_type(bits3-0)
   * buf[4-5] = big-endian length of 5GS Mobile Identity (LV-E)
   * buf[6..] = 5GS Mobile Identity value */
  if (len < 7) return false;
  if (buf[0] != 0x7e || buf[1] != 0x00 || buf[2] != 0x41) return false;

  uint16_t id_len = ((uint16_t)buf[4] << 8) | buf[5];
  if ((size_t)(6 + id_len) > len || id_len < 8) return false;

  const uint8_t *v = buf + 6;
  uint8_t type       = v[0] & 0x07;
  uint8_t supi_fmt   = (v[0] >> 4) & 0x0f;
  /* SUCI = 0x01, SUPI format IMSI = 0x00 */
  if (type != 0x01 || supi_fmt != 0x00) return false;
  /* null protection scheme required for plain MSIN */
  if ((v[6] & 0x0f) != 0x00) return false;

  uint8_t mcc1 = v[1] & 0x0f,  mcc2 = (v[1] >> 4) & 0x0f, mcc3 = v[2] & 0x0f;
  uint8_t mnc1 = v[3] & 0x0f,  mnc2 = (v[3] >> 4) & 0x0f, mnc3 = (v[2] >> 4) & 0x0f;

  /* MSIN starts at v[8]; each byte encodes two BCD digits (lo nibble first) */
  int msin_bytes = id_len - 8;
  char msin[20] = {0};
  int pos = 0;
  for (int i = 0; i < msin_bytes && pos < 19; i++) {
    uint8_t lo = v[8 + i] & 0x0f, hi = (v[8 + i] >> 4) & 0x0f;
    msin[pos++] = '0' + lo;
    if (hi != 0x0f && pos < 19)
      msin[pos++] = '0' + hi;
  }
  ...
  if (mnc3 == 0x0f)
    snprintf(imsi_out, imsi_out_size, "%u%u%u%u%u%s",   mcc1, mcc2, mcc3, mnc1, mnc2, msin);
  else
    snprintf(imsi_out, imsi_out_size, "%u%u%u%u%u%u%s", mcc1, mcc2, mcc3, mnc1, mnc2, mnc3, msin);
  return true;
}

static void write_imsi_to_db(uint64_t ue_id, const char *imsi)
{
  if (!lbo_is_enabled()) return;
  MYSQL *conn = mysql_init(NULL);
  if (!conn) { fprintf(stderr, "[LBO] mysql_init() failed\n"); return; }
  if (!mysql_real_connect(conn, "localhost", "root", "rtlab666", "lbo", 0, NULL, 0)) {
    fprintf(stderr, "[LBO] mysql_real_connect() failed: %s\n", mysql_error(conn));
    mysql_close(conn); return;
  }
  char query[512];
  snprintf(query, sizeof(query),
           "INSERT INTO ue_context (ue_id, imsi) VALUES (%lu, '%s') "
           "ON DUPLICATE KEY UPDATE imsi='%s'",
           ue_id, imsi, imsi);
  if (mysql_query(conn, query))
    fprintf(stderr, "[LBO] UPDATE imsi failed: %s\n", mysql_error(conn));
  else
    LOG_I(NR_RRC, "[LBO] ue_context: ue_id=%lu imsi=%s\n", ue_id, imsi);
  mysql_close(conn);
}

static void write_ue_to_db(uint64_t ue_id, const char *pdu_address)
{
  if (!lbo_is_enabled()) return;
  MYSQL *conn = mysql_init(NULL);
  if (!conn) { fprintf(stderr, "[LBO] mysql_init() failed\n"); return; }
  ...
  char query[512];
  snprintf(query, sizeof(query),
           "INSERT INTO ue_context (ue_id, pdu_address) VALUES (%lu, '%s') "
           "ON DUPLICATE KEY UPDATE pdu_address='%s'",
           ue_id, pdu_address, pdu_address);
  if (mysql_query(conn, query))
    fprintf(stderr, "[LBO] INSERT ue_context failed: %s\n", mysql_error(conn));
  else
    LOG_I(NR_RRC, "[LBO] ue_context: ue_id=%lu pdu_address=%s\n", ue_id, pdu_address);
  mysql_close(conn);
}
...
void rrc_gNB_process_NGAP_PDUSESSION_SETUP_REQ(MessageDef *msg_p, instance_t instance)
{
  ...
  /* Extract PDU address from first NAS PDU (IEI=0x29, IPv4 PDU Address IE)
   * so the upsert below includes the IP immediately at session setup time. */
  char ue_pdu_ip[16] = {0};
  for (int _i = 0; _i < msg->nb_pdusessions_tosetup && !ue_pdu_ip[0]; _i++) {
    const pdusession_resource_item_t *_p = &msg->pdusession[_i];
    if (!_p->nas_pdu.buf || _p->nas_pdu.len < 7) continue;
    for (size_t _j = 4; _j + 6 < _p->nas_pdu.len; _j++) {
      if (_p->nas_pdu.buf[_j] == 0x29) {
        uint8_t _ie_len   = _p->nas_pdu.buf[_j + 1];
        uint8_t _pdu_type = _p->nas_pdu.buf[_j + 2] & 0x07;
        if (_ie_len >= 5 && _pdu_type == 0x01 && _j + 2 + _ie_len <= _p->nas_pdu.len) {
          snprintf(ue_pdu_ip, sizeof(ue_pdu_ip), "%u.%u.%u.%u",
                   _p->nas_pdu.buf[_j + 3], _p->nas_pdu.buf[_j + 4],
                   _p->nas_pdu.buf[_j + 5], _p->nas_pdu.buf[_j + 6]);
          LOG_I(NR_RRC, "[UE_CTX] UE %u pdu_address=%s\n", UE->rrc_ue_id, ue_pdu_ip);
          break;
        }
      }
    }
  ...
  /* Seed the generic UE bearer table with the CU UE ID, C-RNTI, and PDU
   * address. CU-UP/GTP-U fills TEIDs and DU IP later. */
  cpe_ue_table_upsert_ue_context(UE->rrc_ue_id, UE->rnti, ue_pdu_ip[0] ? ue_pdu_ip : NULL, "NORMAL_UE");
...
#ifdef E2_AGENT
  /* Diagnostic hex dump of NAS PDU for LBO debugging. */
  if (lbo_is_enabled()) for (int i = 0; i < msg->nb_pdusessions_tosetup; i++) {
    const pdusession_resource_item_t *p = &msg->pdusession[i];
    if (p->nas_pdu.buf == NULL || p->nas_pdu.len < 5) continue;
    char hexbuf[p->nas_pdu.len * 3 + 1];
    for (size_t j = 0; j < p->nas_pdu.len; j++)
      sprintf(&hexbuf[j*3], "%02X ", p->nas_pdu.buf[j]);
    hexbuf[p->nas_pdu.len * 3] = '\0';
    LOG_I(NR_RRC, "[LBO] UE %u PDU session %d NAS PDU (%zu bytes): %s\n",
          UE->rrc_ue_id, p->pdusession_id, p->nas_pdu.len, hexbuf);
  }
#endif /* E2_AGENT */
