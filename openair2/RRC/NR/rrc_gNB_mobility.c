                                 ...
                                 ho_cancel_t cancel)
{
  ...
  LOG_A(NR_RRC,
        "[F1HO_LAT] phase=trigger ue=%u rnti=0x%04x source_assoc=%d target_assoc=%d target_du=%ld target_pci=%d t_us=%ld\n",
        ue->rrc_ue_id,
        ue->rnti,
        source_du ? source_du->assoc_id : -1,
        target_du->assoc_id,
        target_du->setup_req->gNB_DU_id,
        target_du->setup_req->cell[0].info.nr_pci,
        ho_ctx->trigger_us);
  ...
  *ue_agg_mbr = 1000000000 /*bps*/;
  ...
  ho_ctx->f1_setup_req_us = f1ho_now_us();
  LOG_A(NR_RRC,
        "[F1HO_LAT] phase=f1_setup_request ue=%u rnti=0x%04x target_assoc=%d delta_trigger_us=%ld t_us=%ld\n",
        ue->rrc_ue_id,
        ue->rnti,
        target_du->assoc_id,
        ho_ctx->f1_setup_req_us - ho_ctx->trigger_us,
        ho_ctx->f1_setup_req_us);
...
{
  int64_t now = f1ho_now_us();
  UE->ho_context->rrc_reconf_send_us = now;
  LOG_A(NR_RRC,
        "[F1HO_LAT] phase=rrc_reconf_send ue=%u rnti=0x%04x target_rnti=0x%04x f1_setup_resp_to_reconf_us=%ld trigger_to_reconf_us=%ld t_us=%ld\n",
        UE->rrc_ue_id,
        UE->rnti,
        UE->ho_context->target ? UE->ho_context->target->new_rnti : 0,
        UE->ho_context->f1_setup_resp_us ? now - UE->ho_context->f1_setup_resp_us : -1,
        UE->ho_context->trigger_us ? now - UE->ho_context->trigger_us : -1,
        now);
...
static void nr_rrc_f1_ho_complete(gNB_RRC_INST *rrc, gNB_RRC_UE_t *UE)
{
  ...
  UE->ho_context->source_release_cmd_us = f1ho_now_us();
  ...
  LOG_A(NR_RRC,
        "[F1HO_LAT] phase=source_release_cmd ue=%u rnti=0x%04x old_rnti=0x%04x complete_to_release_us=%ld trigger_to_release_us=%ld t_us=%ld\n",
        UE->rrc_ue_id,
        UE->rnti,
        source_ctx->old_rnti,
        UE->ho_context->rrc_reconf_complete_us ? UE->ho_context->source_release_cmd_us - UE->ho_context->rrc_reconf_complete_us : -1,
        UE->ho_context->trigger_us ? UE->ho_context->source_release_cmd_us - UE->ho_context->trigger_us : -1,
        UE->ho_context->source_release_cmd_us);
...
bool nr_rrc_e2_trigger_ho(uint16_t rnti)
{
  gNB_RRC_INST *rrc = RC.nrrrc[0];
  rrc_gNB_ue_context_t *ue_ctx = rrc_gNB_get_ue_context_by_rnti_any_du(rrc, (rnti_t)rnti);
  if (!ue_ctx) {
    LOG_W(NR_RRC, "[E2 CHO] RNTI 0x%04x not found; reject HO trigger\n", rnti);
    return false;
  }
  gNB_RRC_UE_t *ue = &ue_ctx->ue_context;
  nr_rrc_du_container_t *source_du = get_du_for_ue(rrc, ue->rrc_ue_id);
  if (!source_du) {
    LOG_E(NR_RRC, "[E2 CHO] source DU not found for RNTI 0x%04x\n", rnti);
    return false;
  }
  nr_rrc_du_container_t *target_du = find_target_du(rrc, source_du->assoc_id);
  if (!target_du) {
    LOG_E(NR_RRC, "[E2 CHO] no target DU available for RNTI 0x%04x\n", rnti);
    return false;
  }

  /* Wait up to 5 s for RRC Reconf Complete to clear xids / ho_context.
   * RRC thread runs concurrently so this spin-wait doesn't deadlock. */
  for (int i = 0; i < 100; i++) {
    if (ue->ho_context != NULL) { usleep(50000); continue; }
    bool busy = false;
    for (int j = 0; j < NR_RRC_TRANSACTION_IDENTIFIER_NUMBER; j++)
      if (ue->xids[j] != RRC_ACTION_NONE) { busy = true; break; }
    if (!busy) goto trigger;
    usleep(50000);
  }
  LOG_W(NR_RRC, "[E2 CHO] UE %u still busy after 5 s, aborting HO\n", ue->rrc_ue_id);
  return false;

trigger:
  LOG_I(NR_RRC, "[E2 CHO] Triggering F1 HO for UE RNTI 0x%04x\n", rnti);
  nr_rrc_trigger_f1_ho(rrc, ue, source_du, target_du);
  return true;
}
