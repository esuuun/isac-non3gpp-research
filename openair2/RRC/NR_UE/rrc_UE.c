...
static void nr_rrc_prepare_msg3_payload(NR_UE_RRC_INST_t *rrc)
{
  ...
  switch (rrc->ra_trigger) {
    ...
    case BEAM_FAILURE_RECOVERY:
      /* These RA procedures use MAC-level signaling such as CFRA or C-RNTI MAC CE.
       * They do not carry an RRC CCCH payload like RRCSetupRequest in Msg3. */
      LOG_W(NR_RRC,
            "No RRC CCCH Msg3 payload for RA trigger %d; MAC should use CFRA/C-RNTI CE if needed\n",
            rrc->ra_trigger);
      break;
    default:
      LOG_W(NR_RRC,
            "Skip Msg3 payload preparation: RA trigger %d has no RRC Msg3 payload implemented\n",
            rrc->ra_trigger);
      break;
  }
}
...
static void nr_rrc_handle_msg3_indication(NR_UE_RRC_INST_t *rrc, rnti_t rnti)
{
  ...
  switch (rrc->ra_trigger) {
    ...
    case RRC_CONNECTION_REESTABLISHMENT:
      ...
      break;
    case DURING_HANDOVER:
      /* For reconfigurationWithSync/handover RA, Msg3 is not an RRC CCCH request.
       * The target C-RNTI was provided by RRCReconfiguration::newUE-Identity and
       * MAC handles CFRA or C-RNTI MAC CE based RA completion. */
      rrc->rnti = rnti;
      LOG_I(NR_RRC, "[UE %ld][RAPROC] Msg3 indication during handover RA, RNTI 0x%04x\n", rrc->ue_id, rnti);
      break;
    case NON_SYNCHRONISED:
      rrc->rnti = rnti;
      LOG_I(NR_RRC, "[UE %ld][RAPROC] Msg3 indication for non-synchronised RA, RNTI 0x%04x\n", rrc->ue_id, rnti);
      break;
    ...
    case TO_ESTABLISH_TA:
      rrc->rnti = rnti;
      LOG_I(NR_RRC, "[UE %ld][RAPROC] Msg3 indication for TA establishment RA, RNTI 0x%04x\n", rrc->ue_id, rnti);
      break;
    ...
    case BEAM_FAILURE_RECOVERY:
      rrc->rnti = rnti;
      LOG_I(NR_RRC, "[UE %ld][RAPROC] Msg3 indication for beam failure recovery RA, RNTI 0x%04x\n", rrc->ue_id, rnti);
      break;
    default:
                                                   ...
                                                   int gNB_index)
{
  ...
  rrc->ra_trigger = DURING_HANDOVER;
