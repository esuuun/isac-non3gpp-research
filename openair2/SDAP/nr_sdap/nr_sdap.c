...
static void start_sdap_tun_ue_entity(nr_sdap_entity_t *entity, int sock)
{
  DevAssert(entity != NULL);
  ...
  char thread_name[64];
  snprintf(thread_name, sizeof(thread_name), "ue_tun_read_%ld_p%d", entity->ue_id, entity->pdusession_id);
  threadCreate(&entity->pdusession_thread, sdap_tun_read_thread, entity, thread_name, -1, OAI_PRIORITY_RT_LOW);
}

static void *wait_sdap_entity_and_start_tun(void *arg)
{
  pending_sdap_tun_t *pending = arg;
  for (int i = 0; i < 3000; i++) {
    nr_sdap_entity_t *entity = nr_sdap_get_entity(pending->ue_id, pending->pdu_session_id);
    if (entity != NULL && !entity->is_gnb) {
      LOG_I(SDAP,
            "Starting deferred UE TUN for UE %ld PDU session %d after SDAP entity creation\n",
            pending->ue_id, pending->pdu_session_id);
      start_sdap_tun_ue_entity(entity, pending->sock);
      free(pending);
      return NULL;
    }
    usleep(100000);
  ...
  LOG_E(SDAP,
        "Timed out waiting for SDAP entity UE %ld PDU session %d, keeping TUN socket open\n",
        pending->ue_id, pending->pdu_session_id);
  free(pending);
  return NULL;
}
...
void start_sdap_tun_ue(ue_id_t ue_id, int pdu_session_id, int sock)
{
  ...
  if (entity != NULL) {
    start_sdap_tun_ue_entity(entity, sock);
    return;
  }

  pending_sdap_tun_t *pending = calloc(1, sizeof(*pending));
  if (!pending) {
    LOG_E(SDAP,
          "Cannot defer UE TUN start for UE %ld PDU session %d: allocation failed\n",
          ue_id, pdu_session_id);
    close(sock);
    return;
  }
  pending->ue_id = ue_id;
  pending->pdu_session_id = pdu_session_id;
  pending->sock = sock;

  pthread_t waiter;
  int rc = pthread_create(&waiter, NULL, wait_sdap_entity_and_start_tun, pending);
  if (rc != 0) {
    LOG_E(SDAP,
          "Cannot defer UE TUN start for UE %ld PDU session %d: pthread_create failed %d\n",
          ue_id, pdu_session_id, rc);
    close(sock);
    free(pending);
    return;
  }
  ...
  LOG_W(SDAP,
        "SDAP entity missing for UE %ld PDU session %d, deferred UE TUN start\n",
        ue_id, pdu_session_id);
}
