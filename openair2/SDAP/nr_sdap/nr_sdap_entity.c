...
#define MAX_PENDING_QFI 64
static pending_qfi_t pending_qfi[MAX_PENDING_QFI];

static void store_pending_qfi(ue_id_t ue_id, int pdusession_id, uint8_t qfi)
{
  int free_slot = -1;

  for (int i = 0; i < MAX_PENDING_QFI; i++) {
    if (pending_qfi[i].valid && pending_qfi[i].ue_id == ue_id && pending_qfi[i].pdusession_id == pdusession_id) {
      pending_qfi[i].qfi = qfi;
      return;
    }
    if (!pending_qfi[i].valid && free_slot < 0)
      free_slot = i;
  }

  if (free_slot < 0) {
    LOG_W(SDAP, "Pending QFI table full, overwriting slot 0 for UE %lu PDU session %d\n", ue_id, pdusession_id);
    free_slot = 0;
  }

  pending_qfi[free_slot].valid = true;
  pending_qfi[free_slot].ue_id = ue_id;
  pending_qfi[free_slot].pdusession_id = pdusession_id;
  pending_qfi[free_slot].qfi = qfi;
}

static void apply_pending_qfi(nr_sdap_entity_t *entity)
{
  if (entity == NULL)
    return;

  for (int i = 0; i < MAX_PENDING_QFI; i++) {
    if (!pending_qfi[i].valid)
      continue;
    if (pending_qfi[i].ue_id != entity->ue_id || pending_qfi[i].pdusession_id != entity->pdusession_id)
      continue;

    entity->qfi = pending_qfi[i].qfi;
    LOG_I(SDAP,
          "Applied deferred QFI %u for UE %lu PDU session %d\n",
          entity->qfi,
          entity->ue_id,
          entity->pdusession_id);
    pending_qfi[i].valid = false;
    return;
  }
}
  ...
  if (nr_sdap_get_entity(ue_id, pdusession_id)) {
    ...
    if(!is_gnb)
      nr_sdap_ue_qfi2drb_config(existing_sdap_entity,
                                ...
                                has_sdap_rx,
                                has_sdap_tx);
    apply_pending_qfi(existing_sdap_entity);
...
void set_qfi(uint8_t qfi, uint8_t pduid, ue_id_t ue_id)
{
  ...
  if (entity == NULL) {
    store_pending_qfi(ue_id, pduid, qfi);
    LOG_W(SDAP, "SDAP entity missing for UE %lu PDU session %u, deferred QFI %u\n", ue_id, pduid, qfi);
    return;
  }
  entity->qfi = qfi;
  return;
