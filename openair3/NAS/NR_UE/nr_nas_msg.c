...
{
  if (nas->security_container == NULL) {
    LOG_W(NAS, "No NAS security context: sending plain Registration Complete for CPE NAS proxy\n");
    initialNasMsg->length = sizeof(fgmm_msg_header_t);
    initialNasMsg->nas_data = malloc_or_fail(initialNasMsg->length * sizeof(*initialNasMsg->nas_data));
    fgmm_msg_header_t plain_header = set_mm_header(FGS_REGISTRATION_COMPLETE, PLAIN_5GS_MSG);
    int encoded = _nas_mm_msg_encode_header(&plain_header, initialNasMsg->nas_data, initialNasMsg->length);
    if (encoded < 0)
      LOG_E(NAS, "generateRegistrationComplete: failed to encode plain 5GMM header\n");
...
static void handle_pdu_session_accept(uint8_t *pdu_buffer, uint32_t msg_length, int instance)
{
  ...
  uint32_t remaining_len = msg_length > (uint32_t)size ? msg_length - (uint32_t)size : 0;
  uint32_t accept_body_len = iei_len > (uint16_t)decoded ? iei_len - (uint16_t)decoded : remaining_len;
  if (accept_body_len == 0 || accept_body_len > remaining_len)
    accept_body_len = remaining_len;
  if (!decode_pdu_session_establishment_accept_msg(&msg, pdu_buffer + size, accept_body_len))
...
static void generatePduSessionEstablishRequest(nr_ue_nas_t *nas, as_nas_info_t *initialNasMsg, nas_pdu_session_req_t *pdu_req)
{
  ...
  if (nas->security_container == NULL) {
    LOG_W(NAS, "No NAS security context: sending plain PDU Session Establishment Request for CPE NAS proxy\n");
    const uint16_t container_len = req_length;
    const int plain_len = sizeof(fgmm_msg_header_t) + 1 + 2 + container_len;
    initialNasMsg->nas_data = malloc_or_fail(plain_len * sizeof(*initialNasMsg->nas_data));
    initialNasMsg->length = plain_len;

    int off = 0;
    fgmm_msg_header_t plain_header = set_mm_header(FGS_UPLINK_NAS_TRANSPORT, PLAIN_5GS_MSG);
    off += _nas_mm_msg_encode_header(&plain_header, initialNasMsg->nas_data + off, plain_len - off);
    initialNasMsg->nas_data[off++] = 0x01; /* payload container type: N1 SM information */
    initialNasMsg->nas_data[off++] = (container_len >> 8) & 0xff;
    initialNasMsg->nas_data[off++] = container_len & 0xff;
    memcpy(initialNasMsg->nas_data + off, req_buffer, container_len);
    off += container_len;
    DevAssert(off == plain_len);
