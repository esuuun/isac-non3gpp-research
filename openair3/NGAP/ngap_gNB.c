...
/* =========================================================
 * pdu_entries: store NGAP PathSwitchRequest (buffer[1]==0x1D)
 * for later replay to trigger handover via AMF.
 * 0x1D (29) = NGAP procedure code PathSwitchRequest.
 * ========================================================= */
#define MAX_PDU_BUFFERS 100
...
static ty_pdu_entry_t pdu_buffer[MAX_PDU_BUFFERS];
static int            pdu_buffer_index = 0;

static void insert_pdu_entry(int assoc_id, int stream,
                             const uint8_t *buf, uint32_t buf_len)
{
  if (!lbo_is_enabled()) return;
  if (!buf || buf_len == 0 || buf_len > MAX_PDU_LENGTH) return;
  MYSQL *conn = mysql_init(NULL);
  if (!conn) return;
  if (!mysql_real_connect(conn, "192.168.0.140", "root", "rtlab666",
                          "lbo", 0, NULL, 0)) {
    fprintf(stderr, "[NGAP] pdu_entries MySQL: %s\n", mysql_error(conn));
    mysql_close(conn); return;
  }
  const char *q = "INSERT INTO pdu_entries "
                  "(assoc_id,stream,buffer_length,buffer) "
                  "VALUES (?,?,?,?)";
  MYSQL_STMT *stmt = mysql_stmt_init(conn);
  if (!stmt) { mysql_close(conn); return; }
  ...
  unsigned long bin_len = buf_len;
  bind[0].buffer_type   = MYSQL_TYPE_LONG;   bind[0].buffer = &assoc_id;
  bind[1].buffer_type   = MYSQL_TYPE_LONG;   bind[1].buffer = &stream;
  bind[2].buffer_type   = MYSQL_TYPE_LONG;   bind[2].buffer = (void *)&buf_len;
  bind[3].buffer_type   = MYSQL_TYPE_BLOB;   bind[3].buffer = (void *)buf;
  bind[3].length        = &bin_len;
  mysql_stmt_bind_param(stmt, bind);
  if (mysql_stmt_execute(stmt) == 0)
    printf("[NGAP] pdu_entries stored: assoc=%d stream=%d len=%u\n",
           assoc_id, stream, buf_len);
  else
    fprintf(stderr, "[NGAP] pdu_entries INSERT: %s\n", mysql_stmt_error(stmt));
...
void ngap_gNB_handle_sctp_data_ind(sctp_data_ind_t *sctp_data_ind) {
  ...
  /* Store PathSwitchRequest (0x1D) in pdu_entries for HO replay */
  if (sctp_data_ind->buffer_length > 1 &&
      sctp_data_ind->buffer[1] == 0x1D) {
    if (pdu_buffer_index < MAX_PDU_BUFFERS) {
      ty_pdu_entry_t *e = &pdu_buffer[pdu_buffer_index];
      uint32_t copy_len = sctp_data_ind->buffer_length > MAX_PDU_LENGTH
                          ? MAX_PDU_LENGTH : sctp_data_ind->buffer_length;
      e->assoc_id = sctp_data_ind->assoc_id;
      e->stream   = sctp_data_ind->stream;
      e->buffer_length = copy_len;
      memcpy(e->buffer, sctp_data_ind->buffer, copy_len);
      insert_pdu_entry(e->assoc_id, e->stream, e->buffer, copy_len);
      pdu_buffer_index++;
    }
  }
#endif
